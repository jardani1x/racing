...
| Smoke | `Saved/Automation/veh011-smoke` | `succeeded=528 succeededWithWarnings=2 failed=0 notRun=0` — unchanged from the `RACE-006` baseline; the two warnings are the pre-existing, unrelated `TRACK-001`/`TRACK-002` tests |

**Bypass proof.** `ApplyChassisSleepPin(TEXT("after a reset"))` at `RacingVehiclePawn.cpp:1040`
was commented out, the editor rebuilt (`Scripts/Test/build-veh011-bypass.log`, `Result:
